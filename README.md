...
一个面向情绪困扰时刻的桌面 Web Demo。它不诊断、不催促用户振作，而是通过可响应的虚渺空间，帮助用户命名情绪、允许情绪存在，并找到一件仍然在乎的事。
...
- 四个低负担问题只生成情绪候选词，最终判断权始终属于用户。
- Canvas 粒子空间会根据情绪改变色温、重力、速度和扰动。
- 麦克风只在浏览器本地分析音量，不录音、不转写、不上传。
- 用户可以跳过麦克风、开启减少动态，或随时进入真人帮助页。
